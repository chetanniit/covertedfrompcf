...
:DEFAULT_REGISTRY
    ...
    SET REGISTRY_NAMESPACE=sample-app
	GOTO DOCKER_CONTAINER_RUNTIME
...
echo "building and pushing image simple-pcf-application-buildstage"
pushd source
IF  "%CONTAINER_RUNTIME%" == "docker" (
    docker buildx build --platform ${PLATFORMS} -f Dockerfile.buildstage  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/simple-pcf-application-buildstage . 
) ELSE ( 
    podman manifest create ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/simple-pcf-application-buildstage
    podman build --platform ${PLATFORMS} -f Dockerfile.buildstage --manifest ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/simple-pcf-application-buildstage .
    podman manifest push ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/simple-pcf-application-buildstage
)
popd
echo "building and pushing image simple-pcf-application"
pushd source
IF  "%CONTAINER_RUNTIME%" == "docker" (
    docker buildx build --platform ${PLATFORMS} -f Dockerfile  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/simple-pcf-application . 
) ELSE ( 
    podman manifest create ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/simple-pcf-application
    podman build --platform ${PLATFORMS} -f Dockerfile --manifest ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/simple-pcf-application .
    podman manifest push ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/simple-pcf-application
)
popd
